...
!    UPDATED BY LIHWA LIN, USACE ERDC, 20 May, 2013             C
!***************************************************************C
!---------------------------------------------------------------
...
!
    if(noptset.ne.3)then
      print*,'CMS-Wave V-3.2, last update 20 May 2013'
      print*,'***************************************'
        ...
        do 137 l=1,500
          ...
          do 138 k=1,75
            ...
            if(text(k:kk4).eq.'!free') then
              ...
              read(text,*)nest
                if(nest.ge.1) then
                  !read (11,*) ((inest(nn),jnest(nn)),nn=1,nest) !Commented (Alex Sanchez 12/2/14)
                  read (11,*) (inest(nn),jnest(nn),nn=1,nest)    !Replaces above commented line (Alex Sanchez 12/2/14)
                  do nn=1,80
                    read(11,'(a150)',end=33) text
                    if(text(1:10).ne.'          ') then
                      backspace(11)
...
!$    NTHR = omp_get_max_threads()
      ...
      ENDIF
      if(noptset.ne.3)then
!$    if(IPROC.gt.1) call omp_set_num_threads(IPROC) !Alex, set in CMS-Flow (initia.f90)
      endif
!
...
!
      ...
      if(kout.ge.1) open (10, file = EngOutFile, status = 'unknown')
        open (12, file = ObsFile,status='unknown')
      end if
!
      dindex=0.
...
!
      ...
      if(irunup.ge.1.and.depmin.lt.-.01) then
          do j=1,nj
          do i=1,ni
            if(dep0(i,j).lt..05.and.dep0(i,j).gt.-3.) then
            ijstruc2=ijstruc2+1
            istruc2(ijstruc2)=i
            jstruc2(ijstruc2)=j
...
!
      inquire(file='total.wav',exist=getfile11)
      inquire(file='swell.wav',exist=getfile12)
      if(getfile11) iwet=-2
      if(getfile12) iwet=-3
...
!
          if(SimFile(l-13:l-12).eq.'\1') then
            if(iwet.ne.-1) then
            open(95,file='total.wav',status='unknown')
            write (95, *) ni, nj, dmesh
            end if
            if(iwet.ne.-2) then
            open(96,file='swell.wav',status='unknown')
            open(97,file='sea.wav',status='unknown')
            write (96, *) ni, nj, dmesh
            write (97, *) ni, nj, dmesh
            end if
          else
            if(iwet.ne.-1) then
            open(95,file='total.wav',status='unknown',access='append')
            end if
            if(iwet.ne.-2) then
            open(96,file='swell.wav',status='unknown',access='append')
            open(97,file='sea.wav',status='unknown',access='append')
            end if
          end if
        else
        if(iwet.ne.-1) then
...
!
      itms = 0
      imod = 0
      if(iprpp.eq.1.and.iwind.ge.1) then
        iwind=0
        close(27)
      end if
      if(iwnd.eq.1) then
...
!     two above lines commented by Lihwa 04jun09
      if(iprp.eq.-1.and.mdd.eq.35) then
        write(*,*) '********************** '
        ...
        do mm=1,mdd
          dcm(mm)=0.
          do nn=1,nf
            dcm(mm)=dcm(mm)+dsfd(nn,mm)
          end do
          cc=cc+dcm(mm)
          cc1=cc1+dcm(mm)*(dth*mm-hpai)
          cc2=cc2+dcm(mm)*(dth*mm-hpai)**2
        end do
        cc2=cc2/(cc+.000001)
        cc1=cc1/(cc+.000001)
        cc2=sqrt(abs(cc2-cc1**2))
!       write(*,*) 'cc1,cc2=', cc1/rad,cc2/rad
        md2=(md+1)/2

        if(cc2.lt..2.and.ws.lt..1) then
          ic=int((hpai+cc1)/dth)
          if(ic.lt.1) ic=1
          if(iabs(ic-18).lt.7) then
            imd=ic-3
            if(imd.lt.1) imd=1
            md=imd+6
            if(md.gt.35) md=35
...
!
        ydis=0.
        if(iplane.eq.2) ydis=dvaryyt+dvaryy(nj)
        do j=1,nj
          if(iplane.le.1) then
            ...
            ydis=ydis-dvaryy(nj+1-j)
            xcc=x0+ydis*sinaz+dvarxxt*cosaz
            ycc=y0+ydis*cosaz-dvarxxt*sinaz
          end if
!
          sum=0.
          sum1=0.
...
!
          ...
          dcd(j)=sum2/sum
! 612     continue
!         write(*,*) 'j,wcc=',j,xcc,ycc,wcc(j),dcd(j)
        ENDDO  !611 continue
!
        cc=azimnest-azimuth
        wd=wd+cc
...
!
      ...
      if(hs1.gt..01.and.ibnd.eq.0) then
        ...
        hsmax=ws**1.5/sqrt(fp*g)/17.8*cos(wd-wdir)*abs(sin(wd))
! ----- 17.8 = (4.3)^1.5*2 where 2 is a correction factor (2.5 in reality?)
          if(hsmax.gt.hs0) then
          write(*,*) 'Max Windwave(m) =',hsmax,'m, wind speed =',ws
...
!--------------------------------
80    continue
!!      if (ibreak.eq.2) call dfds  !Alex, commented, subroutine has a bug
...
!          sinaz=-sinaz
        ...
        DO J=jbeg,jend,jinc 
          DO I=ibeg,iend,iinc
            ...
            IF (irs .gt. 0) THEN
              radstr(L)  =WXRS(I,J)*cosaz-WYRS(I,J)*sinaz
              radstr(L+1)=WXRS(I,J)*sinaz+WYRS(I,J)*cosaz
            ENDIF
            IF (IBREAK.EQ.1) THEN
              BRKDISS(K)=IBR(I,J)
...
!
!     + + + PURPOSE + + +
!     Writes "Press <RETURN> to continue..." to the screen and waits
!     for <RETURN> to be pressed before continuing.
!
!----------------------------------------------------------------------F
!
!
!     geostjm: need to verify numargs
!     + + + VARIABLE DECLARATIONS + + +
      CHARACTER*(*)  MSG
          ...
          do l=imd,md
            do k=1,nf
              if (wk2(j, k, l) .gt. 1.0e-10) then
                tkd=min(2.*wk2(j,k,l)*d1(i,j),10.)
                fns=.5*(1.+2.*wk2(j,k,l)*d1(i,j)/sinh(tkd))
...
!
   11 MX=0
      ...
      GOTO 90
!
   50 write(*,*) 'GSR TYPE 1:',II
      GOTO 90
   60 write(*,*) 'GSR TYPE 2:',II,ICC,XMAX
   90 DEALLOCATE (x0)
!
      RETURN
      END
