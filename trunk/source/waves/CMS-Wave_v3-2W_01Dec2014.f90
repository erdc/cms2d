...
!    UPDATED BY LIHWA LIN, USACE ERDC, 15 May, 2014             C
!***************************************************************C
!---------------------------------------------------------------
...
!      refltx(:,:),reflty(:,:)                                   !Wu/Zhang
      ...
      common /dxxdyy/dvarxx(ipmx),dvaryy(ipmx)                   !Wu
      ...
      common /FileNames/ OptsFile, DepFile, CurrFile, EngInFile,    &
                         ...
                         SeaFile, SwellFile                            !Mitch 3/22/2017
...
! ... Output/Input variable
      ...
      CHARACTER*180 SpecFile, XMDFFile,SetupFile
...
!      ALLOCATE (fsp(npf),dep0(ipmx,jpmx),dvarxx(ipmx),dvaryy(ipmx))        !Wu/Zhang
!      ALLOCATE (xc(komx),yc(komx),wc(komx),wcd(komx),hs13(komx))           !Wu/Zhang
!      ALLOCATE (refltx(ipmx,jpmx),reflty(ipmx,jpmx))                       !Wu/Zhang
      getfile5=.false.  !Replaced 'F' (Brad Johnson 12/01/2014)
                                             !Wu
      rk =0.5
      rkr=0.3
      eta=0.0
...
180   continue      
      if(iwind.eq.0) iwnd=0 !Alex, changed iwnd value from 1 to 0
!
        iwind1=0
...
!     options/set-up parameters
      ...
      icur=0
      ibreak=0
      irs=0  !1-Calculate radiation stresses, 2-Calculate wave setup
      kout=0
      ibnd=0
      iwet=0
      ibf=0
...
!     bf=0.001
      ...
      iview=0
      iroll=0
      !Alex, begin, 03/15/2013, section moved here
      iwave=0
      nestin1=1
      nestin2=1
      !Alex, end
        ...
        do 137 l=1,500
          ...
          do 138 k=1,75
            ...
            if(text(k:kk4).eq.'!free') then
              ...
              read(text,*)kout
                if(kout.ge.1) then
                  !read (11,*) ((ijsp(1,nn),ijsp(2,nn)),nn=1,iabs(kout)) !Commented (Brad Johnson 12/2/14)
                  read (11,*) (ijsp(1,nn),ijsp(2,nn),nn=1,iabs(kout))    !Replaces above commented line (Brad Johnson 12/2/14)
                  do nn=1,80
              ...
              read(text,*)nest
                if(nest.ge.1) then
                  ...
                  read (11,*) (inest(nn),jnest(nn),nn=1,nest) !Alex
                  do nn=1,80
                    read(11,'(a150)',end=33) text
                    if(text(1:10).ne.'          ') then
                      backspace(11)
...
!     end if
      if(bf.gt.1.) then
        write(*,*) '*** bed friction index bf > 3, reset to 1 ***'
        if(noptset.eq.3)then
          write(9,*) '*** bed friction index bf > 3, reset to 1 ***'
        endif
        bf=1.
      end if
...
!       if(noptset.eq.3)then
!         write(9,*) '*** backward reflection > 0, reset to 0 ***'
!       endif
!       iarkr=0
!     end if
...
!
    ...
    if(noptset.ne.3)then
      ...
      if(getfile4) then  !Alex, do not want to use previous surge file
...
!
      !Removing this section since the TOTAL.WAV file is no longer needed.  The Default will out both Sea and Swell Files.
...
!
      !Alex, moved up to initialization section
      !iwave=0
      !nestin1=1
      !nestin2=1
      if(iview.ge.1)then
...
!
        ...
        if (SimFile(l-11:l-5).ne.'swsteer') go to 350
          do i=1,iwave1
            do j=1,nestin2
              READ(24,*,err=424,end=424) icc
              READ(24,*,err=424,end=424) ((cc,MM=1,MDD),NN=1,NFF)
            end do
          end do
!
...
!
      !Commented by Alex
      !Variable iwbk not used except in these two lines
      !Also not defined in any common block
      !iwbk=iwvbk
70    continue
      !iwvbk=iwbk
...
!
      if(noptset.eq.3 .and. irs.eq.0) then
        irs = 1
        if(ijstruc2.ge.1) irs=2
        !ibreak=2  !Alex
        !ixmdf=1   !Alex
...
!
      if(ibnd.eq.0) then
        if(noptset.eq.3)then
          READ(8,*,end=42210) eDate,ws,wd,fp,Tide
        else
...
!
      DTH = 180./float(MDD+1)
      if(iview.ge.1) DTH = 180./float(MDD)
      DTH = DTH*RAD
! 
...
435   continue
!
      if(hs0.lt..01.and.abs(ws-2.).lt.1.99) ws=4.
      if(iview.ge.1.and.abs(ws-2.).lt.1.99) ws=4.
!     if(ws.gt.10.) iprp=-1
...
!
          ...
          DTH = 180./float(MD+1)
          if(iview.ge.1) DTH = 180./float(MD)
          DTH = DTH*RAD
          md2=(md+1)/2
          ichoice=2
        ...
        if(imod.eq.0)then
          do i=1,ni
            do j=1,nj
              u1(i,j) = uin(i,j)
              v1(i,j) = vin(i,j)
              if(u1(i,j).lt.-900.0) u1(i,j)=0.0
              if(v1(i,j).lt.-900.0) v1(i,j)=0.0
            enddo
          enddo
        elseif(imod.eq.1)then
          do i=1,ni
...
!
      flat=1.
!!!      if(ws.lt..01.and.fspsum.gt..01) then !Alex, this WRONG. You can't do this. 
...
4100  continue           !Wu/Zhang
      close(11)  
      if(mod(imod+1,2).eq.0)then !Alex
        nc=ni
        ni=nj
        nj=nc
      endif      
...
!********************************************************************************      
      ...
      COMMON /DATD/DX,DY,DXX,DMESH,DTH,kdate,idate,depmax,depmax0
      COMMON /DVAR/DVARX(IGPX),DVARY(JGPX),ETA(IGPX,JGPX),HSK(JGPX)
      COMMON /SPECB/SOP(KOMX,NPF,MPD),SR(2*IPMX,NPF,MPD)
      COMMON /OUTP/KOUT,IJSP(2,KOMX),IRS,IBREAK,ICUR,IWET,INST
      COMMON /OUTN/nest,inest(komx),jnest(komx),ix1(igpx),ix2(igpx)
      COMMON /IJBP/IJB(IGPX,JGPX),DEP(IPMX,JPMX),DEPS(IPMX),DBIG(IPMX)
      COMMON /WAVI/H13(IGPX,JGPX),T13(IGPX,JGPX),DMN(IGPX,JGPX)
      ...
      if (imod .eq. 0 .or. imod .eq. 2) then
        ...
        DO J=jbeg,jend,jinc 
          DO I=ibeg,iend,iinc
            ...
            IF (DIR(K).GT.360.0) DIR(K)=DIR(K)-360.
            ...
            ENDIF
!            depth(K)=d1(i,j)
!            currents(L)  =u1(i,j)*cosaz-v1(i,j)*sinaz
!            currents(L+1)=u1(i,j)*sinaz+v1(i,j)*cosaz
            K=K+1
            L=L+2
          ENDDO              
        ENDDO
        ...
        DO I=ibeg,iend,iinc
          DO J=jbeg,jend,jinc
            ...
            ENDIF
            depth(K)=d1(i,j)
!            currents(L)  =u1(i,j)*cosazz-v1(i,j)*sinazz
!            currents(L+1)=u1(i,j)*sinazz+v1(i,j)*cosazz
            K=K+1
            L=L+2
          ENDDO              
        ENDDO
...
! removed XF_VECTORS_IN_LOCAL_COORDS (DID,ERROR) for proper vector direction
      ...
      ENDIF
      IF(noptset.eq.3) CALL XF_DATASET_REFTIME(DID,REFTIME,ERROR)
      CALL XF_WRITE_SCALAR_TIMESTEP(DID,TIME2,NIJ,surgew, ERROR)
      CALL XF_CLOSE_GROUP (DID, ERROR)
      endif
...
!-----------------------------------------------------
      ...
      ETA=0.
      SOP=0.
      if(iplane.le.1) H13R=0.
      if(iplane.le.1) T13R=t13min
      if(iplane.le.1) DMNR=0.
      HSK=HS0
      H13A=HS0
      H13B=HS0
...
!     cz=.1/tanh(.1*wk2(jj,nn,mm)*d1(ii,jj))
!     cc=si(jj,nn,mm)*tanh(cz*fcn(nn-1)**2)
!     si(jj,nn-1,mm)=si(jj,nn-1,mm)+cc
...
!     IF(SS.LT.1.0E-15) GOTO 40
      if(ss.lt.1.0E-15) ss=1.0E-15
      HSI=cflat*SQRT(SS)
!        goto 410 !Alex, skip wave height check ******* IMPORTANT ***************
        if(iii.gt.1)then !Alex, New segment, wave height is reduced based on wave breaking formulation
          !Maximum waveheight
          ipeak=max(int((dmn(iii,jj)+hpai-dth)/dth+1.5),1)
          wkpeak=abs(wk2(jj,kpeak,ipeak)) 
...
!!	      if(Hrms.ge.1.01*Hmax .and. ibr(iii-1,jj).eq.1)then
	      if(Hrms.ge.1.01*Hmax)then !Official
            ...
            do l=imd,md
              do k=1,nf
                si(jj,k,l)=si(jj,k,l)*cc
              end do
            end do
            Hsi=Hmax*1.414213562
            ibr(iii,jj)=1
          endif
!          if(ibr(iii,jj-1).eq.1 .and. ibr(iii,jj+1).eq.1)then
!            ibr(iii,jj)=1
!          endif          
          
!      else   !Lihwa's segment, wave height is reduced based on depth criteria      
!        edplim=0.64*d1(iii,jj)
!        edplim1=edplim
...
!!
!        if(iii.ge.2) then 
!          if (ibr(iii-1,jj).eq.1) edplim=0.64*d1(iii,jj)
!        end if
!        edplim1=edplim
!        ipeak=int((dmnj(jj)+hpai-dth)/dth+1.5)
!        if (ipeak.eq.0) ipeak = 1
!        wkpeak=abs(wk2(jj,kpeak,ipeak))
!        if(wkpeak.ne.0.) then
!          cc=.31
!          if(d1(iii,jj).lt.hsi) cc=.31*hsi/d1(iii,jj)
!          if(d1(iii,jj).lt.1.1*hsi) cc=cc*hsi/d1(iii,jj)
!          estlim=cc*pai/wkpeak*tanh(wkpeak*d1(iii,jj))
!          if(d1(iii,jj).lt.hs0) cc=cc*d1(iii,jj)/hs0        
...
!          hs00=hs0/3.
!          if(min(hsi,hs00).gt.d1(iii,jj)*1.3*tanh(tp/8.)) ibr(iii,jj)=1
!          cc=(edplim/hsi)**2
...
! -- calculated significant wave height
      H13(III,JJ)=HSI
      if(iplane.eq.1) h13r(iii,jj)=hsi
      if(hsi.gt.hj1) then
      j1=jj
      hj1=hsi
      end if
...
!        if(jj.eq.714) then
!          write(2000,*) 'nn = ',nn,'jj = ',jj,'fcn(nn) = ',fcn(nn),'sjf(jj,nn) = ',sjf(jj,nn),'df(nn) = ',df(nn)
!        endif
      end if
  502 continue
!
...
!          write(1000,*) 'qp = ',qp,'sum2 = ',sum2,'cflat = ',cflat,'iii = ',iii,'jj = ',jj,'h13(iii,jj) = ',h13(iii,jj)
          if(qp.gt.0.0) then
...
! -- repeatition of calculation for next ii location
...
! -- iteration for jj
      ...
      HSB(JJ) = H13(INV,JJ) !Used for wave breaking (Alex Sanchez, 12/05/2014) *********
      HSB(JJ) = H13R(INV,JJ) !Used for wave breaking (Alex Sanchez, 12/05/2014) *********
...
!     end if
      ...
      cc=diss(inv,jj)
      diss(inv,jj)=diss(ii,jj)
      diss(ii,jj)=cc
      k=ibr(inv,jj)
      ibr(inv,jj)=ibr(ii,jj)
      ibr(ii,jj)=k
...
!---------------------------------------------------------------
    ...
    real :: H13,HB,Hrms,X1,X2,PR1,PR2,PX1,PX2,fc,DLHB
    integer :: IBND,IBACK,NBLOCK,IWIND
    real :: TP,PRD,VL,TIDE,KPEAK
    COMMON /DATC/TP,PRD,IBND,VL,TIDE,KPEAK,IBACK,NBLOCK,IWIND
    integer :: kdate,idate
...
! -- ibk=1 and jbv>=6 means no wave breaking
    ...
    slj=slf(jm)  
    if(slj.lt.0.0) return !No wave breaking for negative slopes
...
!   DLHB=A*1.5*PAI*DXX*TANB*SLJ*EXD*ced/2.0
    DLHB=A*0.75*PAI*DXX*TANB*SLJ*EXD*ced    !Alex 0.75 should be 1.5
!    DLHB=A*1.5*PAI*DXX*TANB*SLJ*EXD*ced    !Alex, corrected
    H13=HSB(JJ)
        
    !Check Incident breaking
    Hrms=H13/1.414213562
!    if(Hrms.le.HB .and. ibr(max(ii-1,1),jj).eq.0) return !Alex, only for regular waves
...
!     PX2=PAI*X2/4.0
    ...
    CAB=(PR2-PR1)/(1.0-PR1)*CMN(JM)/DXX !Alex, removed goto
    
    if(cab.gt.1.e-10)then
      diss(ii,jj)=-g*cab*Hrms*Hrms/8.0
      ibr(ii,jj)=1
    else
      diss(ii,jj)=0.0
      ibr(ii,jj)=0
...
!---------------------------------------------------------------
    ...
    real :: sig,ab,dab,dab1,dab2,dab3,tkh
    integer :: kdate,idate
    real :: DX,DY,DXX,DMESH,DTH,depmax,depmax0 !,aslope !Removed variable (Alex Sanchez 12/02/14)
    common /DATD/DX,DY,DXX,DMESH,DTH,kdate,idate,depmax,depmax0 !,aslope(ipmx) !Removed variable (Alex Sanchez 12/02/14)
    integer :: IBK
...
! -- ibk=1 and jbv>=6 means no wave breaking
    !IF(IBK.EQ.1 .or. JBV.GE.6) return !Alex
...
!---------------------------------------------------------------
    ...
    real :: cab,fc,gmh,gm,ALF,SLJ,DLHB
    integer :: kdate,idate
    real :: DX,DY,DXX,DMESH,DTH,depmax,depmax0 !,aslope !Removed variable (Alex Sanchez 12/02/14)
    COMMON /DATD/DX,DY,DXX,DMESH,DTH,kdate,idate,depmax,depmax0 !,aslope(ipmx) !Removed variable (Alex Sanchez 12/02/14)
    integer :: IBK
...
!   gmh=min(gm*pai2*depm(jm)/(0.88*wlmn(jm)),10.)
    ...
    !Check Incident breaking, Alex
    if(Hrms.le.HB.and. ibr(max(ii-1,1),jj).eq.0) return
...
!     PX2=PAI*X2/4.0
    ...
    diss(ii,jj)=-9.806*cab*Hrms*Hrms/8.0 !Alex
    ibr(ii,jj)=1                !Alex
...
!     REAL, ALLOCATABLE :: gxr(:,:),gyr(:,:)
      ...
      ALLOCATE (SPCT(MPD),SPCT1(MPD),dep0(ipmx,jpmx),dsss(ipmx,jpmx))
      ...
      DO 81 J=1,JGMX
        DO 81 I=1,IGMX
          ...
          if(iview.eq.0) then
            ...
            if(iplane.eq.2) then
              cc1=h13(i,j)**2
              cc2=h13r(imax1,jmax1)**2
              H13S(I,j)=SQRT(cc1+cc2)
              cx=cc1*cos(dmn(i,j)*rad)+cc2*cos(dmnr(imax1,jmax1)+pai)
              cy=cc1*sin(dmn(i,j)*rad)+cc2*sin(dmnr(imax1,jmax1)+pai)
              dmn(i,j)=atan2(cy,cx)/rad
              T13(i,j)=(T13(i,j)*cc1+T13R(imax1,jmax1)*cc2)/(cc1+cc2+1.e-10)
            end if
! above 8 lines are for iplane = 2 (iview >= 1)
          end if
!
...
!
      inquire(file='simgrid.dat',exist=getfile18)
      if(getfile18) then
        open(unit=35,file='simgrid.dat',status='old')
          read(35,*,end=190,err=190) x00,y00,az00
        ...
        if(iplane.ne.1.and.ixmdf.eq.0) then
          if(isteer.eq.1.and.idate.lt.10000) then
            ...
            if(idate.le.9999) then
              ...
              !write(98,'(i8,i5)') kdate,idate   !Lihwa
              write(98,'(i8,i10)') kdate,idate !Alex
            end if
          end if
        end if
        end if
...
!
      ...
      if(iplane.eq.1) then
        DO 521 i=igmx,1,-1
        write(68,671) (eta(i,j),j=jgmx,1,-1)
  521   CONTINUE
        DO 522 i=igmx,1,-1
...
!
      ...
      if(isteer.eq.1.and.idate.lt.10000) then
        ...
        if(iwet.ne.-2) then
        write(96,*) iidate  !SeaFile
        write(97,*) iidate  !SwellFile
        end if
      else
       if(kdate.eq.0) then
        ...
        if(iwet.ne.-2) then
          if(idate.le.9999) then
            idate1=mod(kdate,10)*100000+idate
            kdate1=kdate/10
            write(96,'(i7,i6)') kdate1,idate1  !SeaFile
            write(97,'(i7,i6)') kdate1,idate1  !SwellFile
          else
      ...
      if(imod.eq.0) then
       DO 50 J=JGMX,1,-1
        if(depmax.ge.1..and.irs.le.1) then
         ...
         write(66,671) (H13S(I,J),I=1,IGMX)
        end if
         if(iwet.lt.0) then
         if(iwet.ne.-1) write(95,67) (H13S(i,j),i=1,igmx)
         if(iwet.ne.-2) then
         write(96,67)(sw13(i,j),i=1,igmx)  !SeaFile
         write(97,67)(sa13(i,j),i=1,igmx)  !SwellFile
         end if
         end if
      if(irs.ge.2.and.kout.ge.0) write(98,671) (eta(i,j),i=1,igmx)
50    CONTINUE
67    format(16f7.3)
671   format(16f7.3)
      DO 51 J=JGMX,1,-1
        if(depmax.ge.1.) then
         ...
         if(iwet.ne.-2) then
         write(96,68)(tw13(i,j),i=1,igmx)  !SeaFile
         write(97,68)(ta13(i,j),i=1,igmx)  !SwellFile
         end if
         end if
      ...
      if(imod.eq.2) then
       DO 2050 J=1,JGMX
        if(depmax.ge.1..and.irs.le.1) then
         ...
         write(66,671) (H13S(I,J),I=IGMX,1,-1)
        end if
         if(iwet.lt.0) then
         if(iwet.ne.-1) write(95,67) (H13S(i,j),i=igmx,1,-1)
         if(iwet.ne.-2) then
         write(96,67)(sw13(i,j),i=igmx,1,-1)  !SeaFile
         write(97,67)(sa13(i,j),i=igmx,1,-1)  !SwellFile
         end if
         end if
        if(irs.ge.2.and.kout.ge.0) write(98,671)(eta(i,j),i=igmx,1,-1)
2050  CONTINUE
       DO 2051 J=1,JGMX
        if(depmax.ge.1.) then
...
!
      ...
      write(66,671) (H13S(I,J),J=JGMX,1,-1)
      end if
        if(iwet.lt.0) then
      ...
      if(irs.ge.2.and.kout.ge.0) write(98,671)(eta(i,j),j=jgmx,1,-1)
1050  CONTINUE
      DO 1051 I=IGMX,1,-1
      if(depmax.ge.1.) then
...
!
      ...
      write(66,671) (H13S(I,J),J=1,JGMX)
      end if
        if(iwet.lt.0) then
      ...
      if(irs.ge.2.and.kout.ge.0) write(98,671)(eta(i,j),j=1,jgmx)
3050  CONTINUE
      DO 3051 I=1,IGMX
      if(depmax.ge.1.) then
        ...
        DO 120 KK=1,iabs(KOUT)
          ...
          if(kdate.gt.0) then
            if(idate.le.9999) then
              idate1=mod(kdate,10)*100000+idate
              kdate1=kdate/10
              write(10,422) kdate1,idate1,i,j,imod*90,h13s(i,j)
  422     FORMAT(i7,i6,3i5,f8.3)
            else
              WRITE(10,420) kdate,idate,i,j,imod*90,h13s(i,j)
  420     FORMAT(i8,i5,3i5,f8.3)
            end if
          else
          WRITE(10,421) idate,i,j,imod*90,h13s(i,j)
  421     FORMAT(5x,i8,3i5,f8.3)
          end if
          DO 135 N0=1,NFF
          if(ffcn(n0).lt.fcn(1)) then
          write(10,510) (0.,mm=1,mdd)
          go to 135
          end if
          if(ffcn(n0).eq.fcn(1)) then
          IF(ICHOICE.LE.1) THEN
            WRITE(10,510) (SOP(KK,1,MM),MM=1,MDD)
          ELSE
            c0=float(mdd+1)/float(md+1)
            DO mm=1,mdd
            m1=int(float(mm-1)/ammd)+1
          ...
          c1=(ffcn(nff)-ffcn(n0))/(ffcn(nff)-fcn(nf))
          IF(ICHOICE.LE.1) THEN
            WRITE(10,510) (SOP(KK,NF,MM)*c1,MM=1,MDD)
          ELSE
            c0=float(mdd+1)/float(md+1)
            DO mm=1,mdd
            m1=int(float(mm-1)/ammd)+1
            ...
            END DO
            WRITE(10,510) (SPCT(MM)*c1,MM=1,MDD)
          END IF
          go to 135
          end if
!
            ...
            c1=(fcn(nn1)-ffcn(n0))/c0
            IF(ICHOICE.LE.1) THEN
            WRITE(10,510) (SOP(KK,NN,MM)*c1+SOP(KK,NN1,MM)*c2,MM=1,MDD)
            ELSE
            c0=float(mdd+1)/float(md+1)
            DO mm=1,mdd
            m1=int(float(mm-1)/ammd)+1
            ...
            END DO
            WRITE(10,510) (SPCT(MM)*c1+SPCT1(MM)*c2,MM=1,MDD)
            END IF
            go to 135
            end if
  130       CONTINUE
...
!         cc3=cc3/cc2
          ...
          ibr(ii,jj),dsss(ii,jj),cc1
  907     format(i7,i6, 2(1x,i4),f7.3, f6.2, f7.1, &
          2(1x,f7.2, f6.2, f7.1),i6, f7.3,3f7.2)
            else
...
! VARIABLE DECLARATIONS
      ...
      LOGICAL       ExistFile
! OUTPUT FORMATS
2000  FORMAT ('!ERROR - Simulation file "',A,'" not found, use defaults.')
!
!
! EMRL
!!      Count = iargc() !Not supported by PGI
      Count = command_argument_count()
!EMRL AKZFIX

      IF (Count .GT. 0) THEN
        CALL GETARG(1, SimFile)
        IF (.NOT. ExistFile(SimFile)) THEN
          WRITE (*, 2000) TRIM(SimFile)
          SimFile = 'none'
        ENDIF
      ELSE
        SimFile = 'none'
...
!       XMDFFile=trim(ThePath)//XMDFFile(ILOC+1:)//'                   '
        ...
        if(ILOC==0) ILOC=INDEX(SurgeFile,'\',BACK=.TRUE.)
        SurgeFile=trim(ThePath)//SurgeFile(ILOC+1:) //'                   '       

        !Some files get written to another directory if run from within CMS and a different Wave directory is specified - Mitch 03/22/2017
        ILOC=INDEX(SeaFile,'/',BACK=.TRUE.)
        if(ILOC==0) ILOC=INDEX(SeaFile,'\',BACK=.TRUE.)
        SeaFile=trim(ThePath)//SeaFile(ILOC+1:) //'                   '       

        ILOC=INDEX(SwellFile,'/',BACK=.TRUE.)
        if(ILOC==0) ILOC=INDEX(SwellFile,'\',BACK=.TRUE.)
        SwellFile=trim(ThePath)//SwellFile(ILOC+1:) //'                   '       
          ...
          do l=imd,md
            do k=1,nf
              if (wk2(j, k, l) .gt. 1.0e-10) then
                ...
                sumxx=sumxx+cc*cosa(l)*g !Alex
                sumyy=sumyy+cc*sina(l)*g !Alex
              end if
            end do
          end do
        end if
