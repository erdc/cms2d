!
! Note: Presently used for Wave Only, MEB 10/4/2018
!
    MODULE GLOBAL
      parameter (A=0.17)  !A=0.15 suggested by Li's experiments
      PARAMETER (NPF=75,MPD=50,MPD2=75,IPMX=2500,JPMX=2500)
      PARAMETER (IGPX=IPMX-1,JGPX=JPMX-1,MPMX=MPD*JGPX)
      PARAMETER (IJPMX=IPMX*JPMX,IJGPX=IGPX*JGPX)
      PARAMETER (KOMX=20000,NOMX=80000)
    END MODULE 
...
!    UPDATED BY LIHWA LIN, USACE ERDC, 30 Mar, 2018             C
!***************************************************************C
!---------------------------------------------------------------
...
!     dimension refltx(ipmx,jpmx),reflty(ipmx,jpmx)
      ...
      REAL, ALLOCATABLE :: dep0(:,:),dvarxx(:),dvaryy(:),   &
        fsp(:),xc(:),yc(:),wc(:),wcd(:),hs13(:),              &
        refltx(:,:),reflty(:,:),exx(:,:),eyy(:,:)
      ...
      common /FileNames/ OptsFile, DepFile, CurrFile, EngInFile,    &
                         ...
                         SpecFile, WindFile, XMDFFile, ShipFile
...
! ... Output/Input variable
      ...
      CHARACTER*180 SpecFile, XMDFFile, ShipFile
! ... Output file varialbes
!
      ALLOCATE (fsp(npf),dep0(ipmx,jpmx),dvarxx(igpx),dvaryy(jgpx))
      ALLOCATE (xc(komx),yc(komx),wc(komx),wcd(komx),hs13(komx))
      ALLOCATE (refltx(ipmx,jpmx),reflty(ipmx,jpmx))
...
!---- Read names of the STWAVE data files if file name exists
...
#ifdef MERGED_CODE
      call wave_only_print (simfile,iprpp,icur,ibreak,irs,kout,ibnd, &
          iwet,ibf,iark,iarkr,akap,bf,ark,arkr,iwvbk,nonln,igrav,irunup,  &
          imud,iwnd,isolv,ixmdf,iproc,iview,iroll)
#endif      
...
!$omp end parallel                                                      
      IF (IPROC .GT. 1 .and. NTHR.gt.1)THEN
        ...
        IPROC = IPR
      ENDIF
!$    call omp_set_num_threads(IPROC)
!
        ...
        do j=1,nj
          do i=1,ni
            if(dep0(i,j).lt..05.and.dep0(i,j).ge.-3.) then
              ...
              if(ijstruc2.ge.79999) exit
            end if
          end do
        end do
        if(ijstruc2.ge.79999) then
          print*,' '
          print*,'***************************************************'
          print*,'************ WARNING  WARNING  WARNING ************'
          print*,'***************************************************'
          print*,'This version of CMS-Wave is statically dimensioned.'
          print*,'It has a limit of total wave run-up cells  = 80000.'
...
!     write(*,*) 'Wave Input Index =',kdate,idate
      if(kdate.gt.0) then
        if(idate.le.9999) then
          ...
          kdate1=kdate/10
          write(text,'("Wave Input Index = ",i7,i6)') kdate1,idate1
        else
          write(text,'("Wave Input Index = ",i8,i5)') kdate,idate
        end if
      else
        if(idate.lt.itms) idate=itms
        if(idate.lt.iidate) idate=iidate
        write(text,'("Wave Input Index = ",i8)') idate
      end if
#ifdef MERGED_CODE
      call diag_print_message(trim(text))
#else
      write(*,*) trim(text)
#endif      
      write(*,*) ' '
...
!
      if(iark.ge.1) then
        ...
        if(krmx.gt.6*IPMX) then
          write(*,*) 'Total forward reflect cells allowed =',6*IPMX
          write(*,*) 'Need to reduce total reflection cells'
...
!--------------------------------
80    continue
      if (ibreak.ge.2) call dfds
      if (irs.ge.1.and.irs0.eq.0) call rstress
90    continue
!--------------------------------
!     if (ixmdf.ne.2) then
      CALL OUTFILE(dvarxx,dvaryy)
!     endif
!----------------------
!  end of calculation
!----------------------
...
420   continue
      ...
      DEALLOCATE (dep0)
      DEALLOCATE (fsp,xc,yc,wc,wcd)
      DEALLOCATE (refltx,reflty)
      DEALLOCATE (exx,eyy)
...
!********************************************************************************
      ...
      use xmdf
      USE GLOBAL, ONLY: NPF,MPD,MPD2,IPMX,JPMX,KOMX,NOMX,IGPX,JGPX
      REAL, ALLOCATABLE :: height(:),period(:),dir(:),brkdiss(:),radstr(:)
      REAL, ALLOCATABLE :: wave(:),depth(:),surge(:),currents(:)    
      COMMON /VPAI/PAI2,PAI,HPAI,RAD,akap,imod,iprp,island,imd,iprpp     &
                   ,nonln,igrav,isolv,ixmdf,iproc,imud,iwnd,depmin0
      COMMON /DATA/NF,MD,IMAX,JMAX,IGMX,JGMX,JCPB,JCPE,JCB,JCE,NFF,MDD
      COMMON /DATB/ICK3,ICK4,ICHOICE,HS0,wd,ws,ph0,wdd(mpd),aslop(ipmx)
...
! --- ibk=1 and jbv>=6 means no wave breaking
      ...
      R=Hmax/Hrms
      Qb=1.0+0.7523*(R**3+1.5*R)/exp(R**2)-erf(R)
!     0.7523=4/3/sqrt(pi)
...
!     REAL, ALLOCATABLE :: gxr(:,:),gyr(:,:)
      ...
      common /rsb/ wxrs(ipmx,jpmx),wyrs(ipmx,jpmx)
      common /rsm/ cosa(mpd),sina(mpd),d1(ipmx,jpmx)
      common /rsz/ igetfile20,nship,disx(ipmx),disy(ipmx)
      common /ship/ShipL(30),ShipB(30),ShipD(30),ShipS(30)
      common /uv1/u1(ipmx,jpmx),v1(ipmx,jpmx)
      common /struc0/ijstruc1,ijstruc2,ijstruc3,ijstruc4,ismall
      common /struc1/istruc1(komx),jstruc1(komx),dstruc1(komx)
...
290   continue  !end of do loop

291   continue  !plain goto 
!
...
53      continue
      else   !from if(irs.ge.2) conditional
        if(irunup.eq.2) then
          dsss=eta+h13s/2.
        else
...
!
  ...
  391       continue  !out of nested 'do 391' loops
...
!
  ...
  392     continue  !out of nested 'do 392' loops
!
  299     continue
!
        end if
...
252     CONTINUE !out of nested 'do 252' loops
      end if
!
      if(imod.eq.1.or.imod.eq.3) then
        cc=90.*float(imod)
...
253     CONTINUE  !out of nested 'do 253' loops
      end if
!
