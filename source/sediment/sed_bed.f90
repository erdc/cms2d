...
!    !**************************************************************************    
!    use case_size
!    use fl2d, only: idry,zb
!    use bnd2d
!    use comvarbl, only: dtime
!    use flowglobal, only: dgfile,dgunit,mapid
!    use sedmod
!    use precision
!    implicit none
!    integer :: i,ih,ks,js,itg,im,jstar,iwr,irm
!    real(ikind) :: fac,dbdz,dzi,zbi,val
!    real(ikind) :: fterm(nsed),fterm2(nsed),fterm3(nsed)
...
!          pbkstar(i,:)=pbk(i,:,2) !note: pbk(i,ks,2) does not change until bedgrad subroutine   
!       endif
!       Etkp=Etkstar(i,:)/pbk(i,:,1) !kg/m^3
!       aws=alphat(i)*wsfall(:)
!       fterm=dbms(i,1)+dtime*Etkp !kg/m^2
...
!       dmk(i,:)=scalemorph*dmk(i,:) !Apply morphologic scaling factor
!
!       !...Sorting      
!       pbktemp=pbk(i,:,1) !Save old values for convergence test                
!       pbk(i,:,1)=(dmk(i,:)+dbms1(i,1)*pbk1(i,:)+dbdz*pbkstar(i,:))/dbms(i,1)            
!       pbk(i,:,1)=pbk(i,:,1)*rx+pbktemp*(1.0-rx) !Relaxation      
!       where(pbk(i,:,1).lt.1.0e-20) pbk(i,:,1)=1.0e-20 
!       pbksum=sum(pbk(i,:,1)) !sum fraction         
!       pbk(i,:,1)=pbk(i,:,1)/pbksum !normalize    
!       errpbk=max(errpbk,maxval(abs(pbk(i,:,1)-pbktemp))) !check error
!    enddo !i 
!!$OMP END PARALLEL DO
...
!    enddo
!
!    do irm=1,numrubmound     !Rubble Mound Structure, !Wu, Oct. 2011
!       do im=nrubmound(irm-1)+1,nrubmound(irm)
!          i=idrubmound(im)
!          dm(i)=dm(i)/rubmoundporo(irm)
!          dmk(i,:)=dmk(i,:)/rubmoundporo(irm)
!       enddo
!    enddo
!
