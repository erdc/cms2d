    ...
    if(noptset>=3)then  !Waves + Current
      do i=1,ncells
        ...
        if (coefman(i) .eq. 0) then
          fcgr=2.0*(min(cman_p/1.0e-6,1.0))**1.5*cfrict(i)        !If coefman(i) is zero, use very small number instead   MEB 01/12/2022
        else
      ...
      do i=1,ncells
        do ks=1,nsed
          ...
          if (coefman(i) .eq. 0) then
            val = max(0.0, (cman_p/1.0e-6)**1.5*bsxy(i)/taubcr-1.0)   !If coefman(i) is zero, use very small number instead   MEB 01/12/2022
          else
